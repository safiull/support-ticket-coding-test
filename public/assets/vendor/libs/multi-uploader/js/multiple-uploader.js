class MultipleUploader {
    ...
    init( { maxUpload = 10 , maxSize = 2 , formSelector = 'form' , filesInpName = 'images', previousImages = [] } = {} )
    {

        // Check if there are any previous images
        if (previousImages.length > 0) {
            $('.mup-msg').addClass('d-none');
            // Loop through each previous image URL
            previousImages.forEach((imageUrl) => {
                // Create a new image container element
                const imageContainer = document.createElement('div');
                imageContainer.classList.add('image-container');

                // Set the image source to the previous image URL
                imageContainer.innerHTML = `
                    <img src="${$("#public-path").val() + '/' + imageUrl}" class="image-preview" alt="">
                `;

                // Append the image container to the multiple uploader
                this.#multipleUploader.appendChild(imageContainer);
            });
        }
...
let multipleUploader = new MultipleUploader('#multiple-uploader').init({
    previousImages: $.parseJSON($('#prev-images').val()),
    maxUpload : 20, // maximum number of uploaded images
    maxSize:2, // in size in mb
    filesInpName:'galleries', // input name sent to backend
    formSelector: '#property-form', // form selector
});
